...
export class DriverReport implements AfterViewInit, OnDestroy {
  ...
  private renderChartsFromResponse(resp: any) {
    ...
    this.expenseChart = new Chart(ctxExpense, {
      type: 'line',
      data: { labels, datasets: [{ label: 'Earned', data: expenses, borderColor: '#000', backgroundColor: '#000', fill: false, borderWidth: 1 }] },
      options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { color: '#000' } }, y: { ticks: { color: '#000' } } } }
    });
  }
}
