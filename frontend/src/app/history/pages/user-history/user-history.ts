 ...
 private transformRideData(apiData: RideHistoryResponse[]): Ride[] {
  return apiData.map((ride) => {
    ...
    return {
      ...
      pickupAddress: ride.route?.startLocation?.address ?? null,
      destinationAddress: ride.route?.endLocation?.address ?? null,
      stopAddress: (ride.route?.stopLocations && ride.route.stopLocations.length === 1) ? ride.route.stopLocations[0].address : null,
      stopAddresses: ride.route?.stopLocations?.map(s => s.address) ?? null,
