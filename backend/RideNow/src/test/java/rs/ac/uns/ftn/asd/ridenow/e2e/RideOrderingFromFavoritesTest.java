...
public class RideOrderingFromFavoritesTest extends TestBase {
    ...
    void rideOrderingFromFavorites_scheduledTime_pastTimeValidation() {
        ...
        // Try to schedule a ride in the past
        ...
        assertTrue(preferencePage.hasScheduledTimeError(),
                "Error should be shown for past scheduled time");
        String errorMsg = preferencePage.getScheduledTimeError();
        assertNotNull(errorMsg, "Error message should be present");
        assertTrue(errorMsg.toLowerCase().contains("past"),
                "Error message should mention 'past'");

        // Verify order button is disabled
        assertTrue(preferencePage.isOrderButtonDisabled(),
                "Order button should be disabled when scheduled time is in the past");
    }
    ...
    void rideOrderingFromFavorites_scheduledTime_tooFarInFutureValidation() {
        ...
        // Try to schedule a ride more than 5 hours in the future
        ...
        // Verify error message is shown
        assertTrue(preferencePage.hasScheduledTimeError(),
                "Error should be shown for scheduled time too far in future");
        String errorMsg = preferencePage.getScheduledTimeError();
        assertNotNull(errorMsg, "Error message should be present");
        assertTrue(errorMsg.toLowerCase().contains("5 hours") || errorMsg.toLowerCase().contains("ahead"),
                "Error message should mention time limit");

        // Verify order button is disabled
        assertTrue(preferencePage.isOrderButtonDisabled(),
                "Order button should be disabled when scheduled time is too far in future");
    }
}
