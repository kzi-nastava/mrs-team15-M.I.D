...
public class FinishRideRepositoryTest {
    ...
    @Test
    @DisplayName("Driver repository should save driver with updated availability")
    void driverRepository_ShouldSaveDriverWithUpdatedAvailability() {
        // Test saving driver with updated availability (used in finish ride)
        driver.setAvailable(false);
        Driver savedDriver = driverRepository.save(driver);

        assertNotNull(savedDriver);
        assertEquals(driver.getId(), savedDriver.getId());
        assertFalse(savedDriver.getAvailable());

        // Verify persistence
        testEntityManager.flush();
        testEntityManager.clear();

        Driver retrievedDriver = driverRepository.findById(driver.getId()).orElse(null);
        assertNotNull(retrievedDriver);
        assertFalse(retrievedDriver.getAvailable());
    }

    @Test
    @DisplayName("Driver repository should save driver with updated status")
    void driverRepository_ShouldSaveDriverWithUpdatedStatus() {
        // Test saving driver with status changes (used in finish ride)
        ...
        assertNotNull(savedDriver);
        assertEquals(DriverStatus.INACTIVE, savedDriver.getStatus());
        assertNull(savedDriver.getPendingStatus());
        ...
        Driver retrievedDriver = driverRepository.findById(driver.getId()).orElse(null);
        assertNotNull(retrievedDriver);
        assertEquals(DriverStatus.INACTIVE, retrievedDriver.getStatus());
        assertNull(retrievedDriver.getPendingStatus());
    }

    @Test
    @DisplayName("Ride repository should save ride with updated status and end time")
    void rideRepository_ShouldSaveRideWithUpdatedStatusAndEndTime() {
        // Test saving ride with updated status and end time (used in finish ride)
        LocalDateTime endTime = LocalDateTime.now().truncatedTo(java.time.temporal.ChronoUnit.SECONDS);
        requestedRide.setStatus(RideStatus.FINISHED);
        requestedRide.setEndTime(endTime);

        Ride savedRide = rideRepository.save(requestedRide);

        assertNotNull(savedRide);
        assertEquals(RideStatus.FINISHED, savedRide.getStatus());
        assertEquals(endTime, savedRide.getEndTime());

        // Verify persistence
        testEntityManager.flush();
        testEntityManager.clear();

        Ride retrievedRide = rideRepository.findById(requestedRide.getId()).orElse(null);
        assertNotNull(retrievedRide);
        assertEquals(RideStatus.FINISHED, retrievedRide.getStatus());
        // Compare with truncated precision to avoid nanosecond differences
        assertEquals(endTime.truncatedTo(java.time.temporal.ChronoUnit.SECONDS),
                     retrievedRide.getEndTime().truncatedTo(java.time.temporal.ChronoUnit.SECONDS));
    }
